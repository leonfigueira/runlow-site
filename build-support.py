...
<p><strong>Folgen mir meine Einstellungen zwischen den Geräten?</strong> Ja, wenn du es einschaltest: Einstellungen, dann iCloud-Sync. Deine Anmeldungen reisen über deinen eigenen iCloud-Schlüsselbund; Farben, Designs, Namen der Anzeigen und das Layout des Sperrbildschirms über deine eigene iCloud. Seit 1.8.7 kann auch dein Nutzungsverlauf mitreisen, sodass ein neues oder zurückgesetztes Gerät übernimmt, was deine anderen aufgezeichnet haben. Es läuft nichts über Dritte.</p>
...
<p><strong>Mes réglages me suivent-ils d’un appareil à l’autre ?</strong> Oui, si vous l’activez : Réglages, puis Synchronisation iCloud. Vos connexions passent par votre propre trousseau iCloud ; vos couleurs, thèmes, noms de jauges et la disposition de l’écran verrouillé passent par votre propre iCloud. Depuis la 1.8.7, votre historique d’utilisation peut voyager aussi, si bien qu’un appareil neuf ou réinitialisé récupère ce que les autres ont enregistré. Rien ne transite par un tiers.</p>
...
<h2>よくある質問</h2>
<p><strong>メーターが読み込まれなくなりました。</strong>おそらくセッションが終了しています。キーをコピーしたブラウザでclaude.aiからログアウトしたり、Claudeの設定で「すべてのセッションからログアウト」を選んだりすると、キーも終了します。またAnthropic側でも、しばらくすると自動的に期限切れになります。その場合Runlowは「再接続」ボタン付きのバナーを表示します。新しいキーを貼り付ければ、ほかの設定はそのままです。ほかのサービスでは、トークンを貼り直してください。</p>
...
<p><strong>設定は端末間で引き継がれますか。</strong>オンにすればはい。「設定」→「iCloud同期」です。ログインはご自身のiCloudキーチェーンを、色・テーマ・メーター名・ロック画面のレイアウトはご自身のiCloudを通ります。1.8.7からは使用量の履歴も引き継げるので、新しい端末や初期化した端末が、ほかの端末の記録を受け取れます。第三者を経由するものはありません。</p>
...
<h2>常见问题</h2>
<p><strong>用量条不再加载了。</strong>多半是会话已经结束。在你复制密钥的那个浏览器里退出 claude.ai，或者在 Claude 的设置中选择「退出所有会话」，都会让密钥一并失效；另外 Anthropic 也会在一段时间后让它自动过期。这时 Runlow 会显示一条带「重新连接」按钮的提示。粘贴一个新密钥即可，其他设置照旧。其他服务则重新粘贴令牌。</p>
...
<p><strong>我的设置会在设备之间同步吗？</strong>会，只要你打开它：「设置 → iCloud 同步」。登录信息通过你自己的 iCloud 钥匙串，颜色、主题、用量条名称和锁屏布局通过你自己的 iCloud。从 1.8.7 起，用量历史也能一起同步，新设备或重置过的设备可以拿回其他设备记录的内容。不会经过任何第三方。</p>
...
<h2>자주 묻는 질문</h2>
<p><strong>미터가 더 이상 불러와지지 않습니다.</strong> 세션이 끝났을 가능성이 큽니다. 키를 복사한 브라우저에서 claude.ai를 로그아웃하거나 Claude 설정에서 '모든 세션에서 로그아웃'을 고르면 키도 함께 끝나고, Anthropic 쪽에서도 일정 시간이 지나면 스스로 만료시킵니다. 그럴 때 Runlow는 '다시 연결' 버튼이 있는 배너를 보여줍니다. 새 키를 붙여넣으면 나머지는 그대로 유지됩니다. 다른 서비스는 토큰을 다시 붙여넣으세요.</p>
...
<p><strong>설정이 기기 사이를 따라오나요?</strong> 켜 두면 그렇습니다. 설정에서 iCloud 동기화를 여세요. 로그인은 본인의 iCloud 키체인을 통해, 색상·테마·미터 이름·잠금 화면 배치는 본인의 iCloud를 통해 오갑니다. 1.8.7부터는 사용량 기록도 함께 옮겨 다닐 수 있어, 새 기기나 초기화한 기기가 다른 기기의 기록을 이어받습니다. 제3자를 거치는 것은 없습니다.</p>
